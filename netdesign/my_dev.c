...
void InsertHash(int HASH,struct sk_buff *skb,int action){
	...
	//获取当前时间
    	...
    	cur_tm.tm_mon+=1;
    	cur_tm.tm_hour+=8;
		int currenttime=cur_tm.tm_hour*60*60+cur_tm.tm_min*60+cur_tm.tm_sec;//时间戳, 记录当前时间，如果超时则被认为需要删除出TCP，UDP表
		int intervaltime=0;
	...
	// xx=1;
	// sprintf(diary,"Hash has Inserted!\n");
...
//删除HASH节点
void RemoveHash(struct HASHTABLE*head){
	...
	// xx=1;
	// sprintf(diary,"Hash has removed!\n");
	// wake_up_interruptible(&myqueue);
}
//返回1说明找到了并且通过，返回0说明找到了并且按照规则应该拒绝，返回2说明没有找到按规则已经插入
int searchHASH(int HASH,int dst_ip[4],int src_ip[4],int src_port,int dst_port,int protocol,int remove){
	...
	//获取当前时间
    	...
    	cur_tm.tm_mon+=1;
    	cur_tm.tm_hour+=8;
		int currenttime=cur_tm.tm_hour*60*60+cur_tm.tm_min*60+cur_tm.tm_sec;
		int intervaltime=0;
		// xx=1;
		// sprintf(diary,"In SearchHash Function,HASHNUMBER is %d,ip is :%d.%d.%d.%d dip is :%d.%d.%d.%d.\n",HASH,dst_ip[0],dst_ip[1],dst_ip[2],dst_ip[3],
		// src_ip[0],	src_ip[1],	src_ip[2],	src_ip[3]);
		// wake_up_interruptible(&myqueue);
    	//printk("UTC time :%d-%d-%d %d:%d:%d\n",cur_tm.tm_year,cur_tm.tm_mon, cur_tm.tm_mday,cur_tm.tm_hour,cur_tm.tm_min,cur_tm.tm_sec);
	while(head->next!=NULL){
		//xx=1;
		// sprintf(diary,"In SearchHash Function LOOP,HASHNUMBER is %d,ip is :%d.%d.%d.%d dip is :%d.%d.%d.%d.\n",HASH,dst_ip[0],dst_ip[1],dst_ip[2],dst_ip[3],
		// src_ip[0],	src_ip[1],	src_ip[2],	src_ip[3]);
		// wake_up_interruptible(&myqueue);
    	
		head=head->next;
		intervaltime=currenttime-head->currenttime;
		// 		xx=1;
		// sprintf(diary,"Interval time is%d\n",intervaltime);
		// wake_up_interruptible(&myqueue);
		if(intervaltime>=180){
			RemoveHash(head);
		}
		...
		&&(protocol==guize_protocol)){
			if(remove==1){
				xx=1;
				sprintf(diary,",Receive Fin,Hash will be removed!,ip:%d.%d.%d.%d:%d,dstip:%d.%d.%d.%d:%d\n"
				,src_ip[0],src_ip[1],src_ip[2],src_ip[3],src_port,dst_ip[0],dst_ip[1],dst_ip[2],dst_ip[3],dst_port);
				wake_up_interruptible(&myqueue);
				RemoveHash(head);
				if(action==1){
					return 1;
...
//统一使用同一个function:
unsigned int hookfunction(void *priv,
			struct sk_buff *skb,
			const struct nf_hook_state *state){
		ktime_t time;
    	struct rtc_time cur_tm;
	...
	if((This_Protocol==IPPROTO_TCP)||(This_Protocol==IPPROTO_UDP)){
		int result;
		printk("Start Search!\n");
		if((This_Protocol==IPPROTO_TCP)&&(tcph->fin==1)){
				// xx=1;
				// sprintf(diary,",Found fin!\n");
				// wake_up_interruptible(&myqueue);
			result=searchHASH(HASH,DSTIP,SRCIP,srcport,dstport,This_Protocol,1);//找到并且删除报文
		}
		else{
				// 			xx=1;
				// sprintf(diary,"Start Search!\n");
				// wake_up_interruptible(&myqueue);
			result=searchHASH(HASH,DSTIP,SRCIP,srcport,dstport,This_Protocol,0);
		}
		if(result==1){
	...
	for(index=0;index<rule_num;index++){
			...
			if(flag==1){
				xx=1;
				sprintf(diary,"Catched,Here is srcip:%d.%d.%d.%d:%d,dstip:%d.%d.%d.%d:%d\n",SRCIP[0],SRCIP[1],SRCIP[2],SRCIP[3],htons(srcport),
				DSTIP[0],DSTIP[1],DSTIP[2],DSTIP[3],htons(dstport));
				wake_up_interruptible(&myqueue);
...
unsigned int hook_out(void *priv,
	...
	return hookfunction(priv,skb,state);
	...
	size_t size, loff_t *ppos)
{
	...
	wait_event_interruptible(myqueue,xx);
	xx=0;
	if(copy_to_user(buf,diary,sizeof(diary))==0){
		printk("IS READING...");
		return 1;
	}
	else{
